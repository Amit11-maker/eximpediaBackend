...
const createCompanyRecommendation = async (req, res) => {
  ...
  try {
    let favoriteCompanyLimits =
      await recommendationModel.getFavoriteCompanyLimits(payload.account_id);
    ...
    if (favoriteCompanyLimits?.favorite_company_limit?.remaining_limit > 0) {
      ...
      recommendationModel.createCompanyRecommendation(
        ...
        async (error, recommendation) => {
          if (error) {
            ...
            favoriteCompanyLimits.favorite_company_limit.remaining_limit =
              favoriteCompanyLimits?.favorite_company_limit?.remaining_limit -
              1;
            await recommendationModel.updateFavoriteCompanyLimits(
              payload.account_id,
              favoriteCompanyLimits
            );

            res.status(200).json({
              id: recommendation.insertedId,
              consumedCount:
                favoriteCompanyLimits.favorite_company_limit.alloted_limit -
                favoriteCompanyLimits.favorite_company_limit.remaining_limit,
              allotedCount:
                favoriteCompanyLimits.favorite_company_limit.alloted_limit,
            });
      ...
      res.status(409).json({
        message:
          "Max-Favorite-Company-Limit reached... Please contact administrator for further assistance.",
      });
    }
  } catch (error) {
    logger.log(
...
const createShipmentRecommendation = async (req, res) => {
  ...
  try {
    let favoriteShipmentLimits =
      await recommendationModel.getFavoriteShipmentLimits(payload.account_id);
    ...
    if (favoriteShipmentLimits?.favorite_shipment_limit?.remaining_limit > 0) {
      ...
      recommendationModel.createShipmentRecommendation(
        ...
        async (error, shipment) => {
          if (error) {
            ...
            favoriteShipmentLimits.favorite_shipment_limit.remaining_limit =
              favoriteShipmentLimits?.favorite_shipment_limit?.remaining_limit -
              1;
            await recommendationModel.updateFavoriteShipmentLimits(
              payload.account_id,
              favoriteShipmentLimits
            );

            res.status(200).json({
              id: shipment.insertedId,
              consumedCount:
                favoriteShipmentLimits.favorite_shipment_limit.alloted_limit -
                favoriteShipmentLimits.favorite_shipment_limit.remaining_limit,
              allotedCount:
                favoriteShipmentLimits.favorite_shipment_limit.alloted_limit,
            });
      ...
      res.status(409).json({
        message:
          "Max-Favorite-Shipment-Limit reached... Please contact administrator for further assistance.",
      });
    }
  } catch (error) {
    logger.log(
...
const updateCompanyRecommendation = async (req, res) => {
  ...
  try {
    let favoriteCompanyLimits =
      await recommendationModel.getFavoriteCompanyLimits(req.user.account_id);
              ...
              async (error, updateCount) => {
                if (error) {
                  ...
                  res.status(200).json({
                    ...
                    consumedCount:
                      favoriteCompanyLimits.favorite_company_limit
                        .alloted_limit -
                      favoriteCompanyLimits.favorite_company_limit
                        .remaining_limit,
                    allotedCount:
...
const updateShipmentRecommendation = async (req, res) => {
  ...
  try {
    const shipmentRecommendationData =
      recommendationSchema.fetchRecommendationSchema(payload);
    let favoriteShipmentLimits =
      await recommendationModel.getFavoriteShipmentLimits(req.user.account_id);
        ...
        if (error) {
          ...
          if (results.length > 0) {
            ...
            if (updateShipment.isFavorite == true) {
              favoriteShipmentLimits.favorite_shipment_limit.remaining_limit =
                favoriteShipmentLimits?.favorite_shipment_limit
                  ?.remaining_limit - 1;
            } else {
              favoriteShipmentLimits.favorite_shipment_limit.remaining_limit =
                favoriteShipmentLimits?.favorite_shipment_limit
...
const relatedSearch = async (req, res) => {
  ...
  try {
    const companyData = await TradeController.fetchCompanyDetails(
      req,
      res,
      true
    );
...
const recommendationSearch = async (req, res) => {
  ...
  try {
    ...
    const searchRecommendationOutput =
      await recommendationModel.fetchSearchRecommendation(payload);
...
const fetchRecommendationByValue = async (req, res) => {
  ...
  try {
    ...
    recommendationModel.findTradeShipmentRecommendationByValueAggregationEngine(
      payload,
      dataBucket,
...
const sendCompanyRecommendationEmail = async (
  data,
  resultCount,
  companyName
) => {
    ...
    for (let company of companies) {
      if (company.isFavorite === true) {
        ...
        if (
          CDR_endDate != "" &&
          mail_endDate != "" &&
          CDR_endDate != undefined &&
          mail_endDate != undefined &&
          CDR_endDate != mail_endDate
        ) {
          let esCount = await fetch_esCount(
            esMetaData,
            CDR_endDate,
          ...
          if (esCount.body.count > 0) {
            ...
            if (updateCount.modifiedCount > 0) {
              ...
              let result = await NotificationModel.add(
                favoriteCompanyNotifications,
                notificationType
              );
              let mailResult = await sendCompanyRecommendationEmail(
                userDetails,
                esCount,
                esMetaData.columnValue
              );
...
const fetchCDR_EndDate = async (taxonomy_id) => {
  try {
    let country_date_range =
      recommendationSchema.fetchCDNRecommendationSchema(taxonomy_id);
    let countryDateRangeResults =
      await recommendationModel.findCountryDateRangeEndDate(country_date_range);
...
const fetchMail_EndDate = async (user_id, favorite_id) => {
  try {
    let recommendationMail = recommendationSchema.fetchRecommendationMailSchema(
      user_id,
      favorite_id
    );
    let recommendationMailResults =
      await recommendationModel.findRecommendationEmailEndDate(
        recommendationMail
      );
...
const updateMail_EndDate = async (id, CDR_endDate) => {
  try {
    let recommendationEmailUpdateData =
      recommendationSchema.updateRecommendationEmailSchema(id, CDR_endDate);
    let result = await recommendationModel.updateRecommendationEmail(
      recommendationEmailUpdateData
    );
    return result;
  } catch (e) {
    logger.log(
...
const fetch_esCount = async (esMetaData, CDR_endDate, mail_endDate) => {
  try {
    let esData = recommendationSchema.esSchema(
      esMetaData,
      CDR_endDate,
      mail_endDate
    );
    ...
    if (esResults) {
      ...
      logger.log("cannot fetch data from elastic search");
    }
  } catch (e) {
    logger.log(
      ` RECOMMENDATION CONTROLLER ================== ${JSON.stringify(e)}`
...
const insertMail_EndDate = async (data, CDR_endDate) => {
  try {
    let emailData = recommendationSchema.addRecommendationEmailSchema(
      data,
      CDR_endDate
    );
  ...
  onTick: async () => {
    try {
      if (process.env.MONGOCLUSTER != "eximpedia-dev.dhtuw.mongodb.net") {
        ...
        if (users.length < 0) {
          logger.log(
            "RECOMMENDATION CONTROLLER ================== NO data found"
          );
        } else {
