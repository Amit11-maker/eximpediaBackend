...
const updateConstraints = (req, res) => {
  ...
  OrderModel.updateItemSubscriptionConstraints(accountId, subscriptionId, accountPlanConstraint.plan_constraints, (error, orderUpdateStatus) => {
    if (error) {
      res.status(500).json({
        message: 'Internal Server Error',
      });
      ...
      if (orderUpdateStatus) {
        accountPlanConstraint.plan_constraints.order_item_subscription_id = ObjectID(subscriptionId);
        AccountModel.update(accountId, accountPlanConstraint, (error, accountUpdateStatus) => {
          if (error) {
            res.status(500).json({
              message: 'Internal Server Error',
...
const fetchSubscriptions = (req, res) => {
  ...
  SubscriptionModel.findByAccount(payloadParams.accountId, null, offset, limit, (error, subscriptionDataPack) => {
    if (error) {
      res.status(500).json({
        message: 'Internal Server Error',
      });
      ...
      if (!subscriptionDataPack) {
        ...
        let recordsTotal = (subscriptionDataPack[SubscriptionSchema.RESULT_PORTION_TYPE_SUMMARY].length > 0) ? subscriptionDataPack[SubscriptionSchema.RESULT_PORTION_TYPE_SUMMARY][0].count : 0;
      ...
      bundle.data = subscriptionDataPack[SubscriptionSchema.RESULT_PORTION_TYPE_RECORDS];
