...
const create = async (req, res) => {
  try {
    ...
    let createNotification = await NotificationModel.add(
      notificationDetails,
      notificationType
    );
    if (createNotification.insertedId) {
      ...
      logger.log(JSON.stringify(createNotification));
    }
  } catch (error) {
    logger.log(
...
async function fetchNotification(req, res) {
  try {
    let generalNotifications =
      await NotificationModel.getGeneralNotifications();
    let accountNotifications = await NotificationModel.getAccountNotifications(
      req.user.account_id
    );
    let userNotifications = await NotificationModel.getUserNotifications(
      req.user.user_id
    );

    let notificationsArr = {};
    generalNotifications.sort((a, b) =>
      a.created_at < b.created_at ? 1 : b.created_at < a.created_at ? -1 : 0
    );
    accountNotifications.sort((a, b) =>
      a.created_at < b.created_at ? 1 : b.created_at < a.created_at ? -1 : 0
    );
...
const checkExpiredAccount = async (account) => {
  if (account?.plan_constraints?.access_validity_interval?.end_date) {
    if (
      (new Date(account?.plan_constraints?.access_validity_interval?.end_date) -
        new Date()) /
        86400000 <=
      15
    ) {
      ...
      let expireNotification = await NotificationModel.add(
        notificationInfo,
        notificationType
      );
...
const dataAdditionJob = new CronJob({
  ...
  onTick: async () => {
    try {
      ...
      logger.log(
        `NOTIFICATION CONTROLLER ================== ${JSON.stringify(e)}`
      );
      throw e;
    }
  },
