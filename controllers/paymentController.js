...
const create = (req, res) => {
  ...
  if (payment.error == null && payment.transaction_signature != null) {
    ...
    if (PaymentHelper.verifyTransactionSignature(payload.order_ref_id, payload.transaction_id, payload.transaction_signature, PaymentHelper.RAZORPAY_CREDENTIALS.key_secret)) {
      ...
      SubscriptionModel.findByOrderId(payload.order_id, null, function (error, subscriptionBundle) {
        ...
        if (error) {
          ...
          subscriptionConstraints.subscribed_ts = subscriptionConstraints.created_ts;

          PaymentModel.addForOrder(payload.order_id, payload.order_ref_id, order_status, subscriptionConstraints, payment, (error, paymentModifedStatus) => {
            if (error) {
              ...
              if (paymentModifedStatus) {
                subscriptionConstraints.order_item_subscription_id = subscription._id;
                let planConstraints = {
      ...
      PaymentModel.addForOrder(payment.order_id, payment.order_ref_id, OrderSchema.PROCESS_STATUS_UNKNOWN, {}, payment, (error, paymentModifedStatus) => {
        if (error) {
          res.status(500).json({
            message: 'Internal Server Error',
          });
    ...
    PaymentModel.addForOrder(payment.order_id, payment.order_ref_id, OrderSchema.PROCESS_STATUS_FAILED, {}, payment, (error, paymentModifedStatus) => {
      if (error) {
        res.status(500).json({
          message: 'Internal Server Error',
        });
