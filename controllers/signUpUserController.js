...
function sendActivationMail(accountID, userID, res) {
  UserModel.findById(userID, null, (error, user) => {
    if (error) {
      logger.log(
        ` SIGNUP USER CONTROLLER ================== ${JSON.stringify(error)}`
      );
      res.status(500).json({
      ...
      if (!user.is_email_verified) {
        let templateData = {
          activationUrl:
            EnvConfig.HOST_WEB_PANEL + "password/reset-link?id" + "=" + userID,
        ...
        EmailHelper.triggerEmail(emailData, (error, mailtriggered) => {
          if (error) {
            logger.log(
              ` SIGNUP USER CONTROLLER ================== ${JSON.stringify(
                error
              )}`
            );
            res.status(500).json({
            ...
            if (mailtriggered) {
              ...
              res.status(200).json({
                data: {
                  customer_id: accountID,
                  message:
                    "Issue in trigerring mail , please reset password from login page",
                },
              });
            }
          }
...
const validateEmailId = (req, res) => {
  ...
  if (!emailID) {
    logger.log(
      "SIGNUP USER CONTROLLER ================== NO EMAIL ID FOUND OR THE PROVIDED IS INVALID"
    );
    res.status(500).json({
      data: {
        type: "Server Error",
...
const getUserPlanDetails = async (req, res) => {
  ...
  try {
    ...
    if (orderDetails.length == 0) {
      ...
      plan.access_validity_interval =
        orderDetails[0].items[0].meta.access_validity_interval;
      plan.data_availability_interval =
        orderDetails[0].items[0].meta.data_availability_interval;
...
const planRequest = async (req, res) => {
  ...
  try {
    ...
    if (orderDetails.length == 0) {
      ...
      orderDetails[0].items[0].meta.subscriptionType != payload.plan_type
    ) {
      (orderDetails[0].items[0].orderID = randomstring
        .generate({ length: 6, charset: "alphanumeric" })
        .toString()
        .toUpperCase()),
    ...
    if (orderDetails.length != 0) {
      ...
      OrderModel.update(orderDetails[0]._id, orderDetails[0], (error) => {
        if (error) {
          ...
          if (
            orderDetails[0].status == signUpUserSchema.PROCESS_STATUS_SUCCESS
          ) {
            AccountModel.update(
              accountID,
            ...
            res.status(200).json({
              customer_id: accountID,
              message:
                "Payment status was failed , constraints not applied. Try again !!",
            });
          }
        }
      });
