...
const applyQueryMatchExpressions = (data) => {
  let queryMatch = queryMatchExpressions.filter(
    (expression) => expression.type == data.expressionType
  )[0];
  //logger.log(queryMatch);
  ...
  if (data.fieldValueLeft != null && data.fieldValueRight != null) {
    if (data.fieldValueLeft != undefined && data.fieldValueRight != undefined) {
      ...
      query = query.replace(
        /XXX_FIELD_VALUE_RIGHT_XXX/gi,
        data.fieldValueRight
      );
    }
  }
...
const applyQueryProjectionExpressions = (data) => {
  let queryProjection = queryProjectionExpressions.filter(
    (expression) => expression.type == data.expressionType
  )[0];
