...
$(document).ready(function () {
  ...
  $(document).on("click", "body", function (e) {
    if (
      $(e.target).closest(".right-bar-action-data-column-toggle").length > 0
    ) {
      return;
    }
    if (
      $(e.target).closest(
        ".right-bar-filter-country-toggle, .right-bar-filter-records-toggle, .right-bar-filter-year-toggle, .right-bar-filter-month-toggle, .right-bar"
      ).length > 0
    ) {
      return;
    }
  ...
  function buildLedgerBundle(ledgerBundlePack) {
    if (ledgerBundlePack.data.length === 0) {
      ...
      if (
        ledgerBundlePack.data[0].summary.length === 0 ||
        ledgerBundlePack.data[0].ledger_files.length === 0
      ) {
        detachSummarization();
        freshenDTList();
        Swal.fire({
