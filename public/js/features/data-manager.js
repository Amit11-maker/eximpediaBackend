...
function accessAcceptedFilePack() {
  ...
  accessRawFiles().forEach(rawFile => {
    accessAcceptedFiles().forEach(accessAcceptedFile => {
      if (rawFile.name === accessAcceptedFile.name) accessAcceptedFilePackArr.push(rawFile);
    });
  });
...
function accessAcceptedFilePackEntry(rawfileUUID) {
  let accessAcceptedFilePackEntryArr = accessAcceptedFilePack().filter(accessAcceptedFile => (accessAcceptedFile.upload.uuid === rawfileUUID));
...
$(document).ready(function () {
  ...
  function prepareImportCountryOptions() {
    ...
    taxonomyStandards.forEach(taxonomy => {
      if (!addedOptionsArr.includes(taxonomy.code_iso_3)) {
        let option = `<option value="${taxonomy.code_iso_3}">${taxonomy.country.toUpperCase().trim()}</option>`;
  ...
  function accessTaxonomyStandard(taxonomyId) {
    let taxonomyStandardsArr = taxonomyStandards.filter(taxonomyStandard => (taxonomyStandard._id === taxonomyId));
  ...
  function updateUploadStatusBlock(fileId, status) {
    ...
    let stageStatusArr = statusBlockConfig.filter(stageConfig => stageConfig.status === status);
    if (stageStatusArr.length > 0) stageStatus = stageStatusArr[0];
    if (stageStatus) {
      $(`#fileUploadCard table tbody tr#${fileId} td.label_status .progress-loader`).hide();
      $(`#fileUploadCard table tbody tr#${fileId} td.label_status .content`).html(`
      <span class="badge bg-soft-${stageStatus.indicatorVariant} text-${stageStatus.indicatorVariant} p-1 shadow-none">${stageStatus.text}</span>`).show();
    }
  }
    ...
    if (stageStatus) {
      switch (status) {
        case IMPORT_STATUS_PENDING: {
          $(`#fileUploadCard table tbody tr#${fileId} td.label_progress .progress-loader`).hide();
          $(`#fileUploadCard table tbody tr#${fileId} td.label_progress .content`).show();
          break;
        }
        case IMPORT_STATUS_FAILED: {
          $(`#fileUploadCard table tbody tr#${fileId} td.label_progress .progress-loader`).hide();
          $(`#fileUploadCard table tbody tr#${fileId} td.label_progress .content`).html(`
             <span class="badge bg-soft-${stageStatus.indicatorVariant} text-${stageStatus.indicatorVariant} p-1 shadow-none">${stageStatus.text}</span>`).show();
          break;
        }
        default:
  ...
  function updateUploadProgressCounter(fileId, progressCounter) {
    $(`#fileUploadCard table tbody tr#${fileId} td.label_progress .content .progress-counter`).text(`${progressCounter}%`);
    $(`#fileUploadCard table tbody tr#${fileId} td.label_progress .content .progress-bar`).
    attr('aria-valuenow', `${progressCounter}`).css('width', `${progressCounter}%`);
  }
  ...
  function updateUploadActionBlock(fileId, status) {
    ...
    if (stageStatus) {
      switch (status) {
        case IMPORT_STATUS_PENDING: {
          $(`#fileUploadCard table tbody tr#${fileId} td.label_action .progress-loader`).hide();
          $(`#fileUploadCard table tbody tr#${fileId} td.label_action .content`).show();
          break;
        }
        case IMPORT_STATUS_FAILED: {
          $(`#fileUploadCard table tbody tr#${fileId} td.label_action .progress-loader`).hide();
          $(`#fileUploadCard table tbody tr#${fileId} td.label_action .content`).html(`
  ...
  function updateUploadSyncProcess(fileId) {
    ...
    if (fileEntry.status === IMPORT_STATUS_COMPLETED) {
      ...
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .progress-loader`).hide();
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content .title`).text('Upload Success');
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content .summary`).html(
        '<small class="summary_passed">No Issues Detected</small>');
      ...
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content`).show();
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content .title`).text('Upload Failed');
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content .summary`).html(
  ...
  function uploadFile(fileId) {
    ...
    var params = {
      Key: encodeURIComponent(mediaStoreOptions.rootFolder).concat('/')
        .concat(encodeURIComponent(mediaStoreOptions.containerFolder)).concat('/').concat(fileEntry._id),
      Metadata: meta,
      Body: uploadFile
      //ACL: 'public-read'
    };
    ...
    S3Client.upload(params, function (err, data) {
      if (err) {
        ...
        for (let i in fileImportQueue) {
          if (fileImportQueue[i].stage === IMPORT_STAGE_UPLOAD && fileImportQueue[i].status === IMPORT_STATUS_PENDING) {
  ...
  function updateUploadValidationProcess(fileId) {
    ...
    $(`#fileUploadCard table tbody tr#${fileId} td.label_records`).text(fileEntry.trade_records);
    ...
    if (fileEntry.status === IMPORT_STATUS_COMPLETED) {
      ...
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content`).show();
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content .title`).text('Formats Passed');
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content .summary`).html(
        '<small class="summary_passed">No Issues Detected</small>');
      ...
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content`).show();
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content .title`).text('Formats Failed');
      $(`#fileUploadCard table tbody tr#${fileId} td.label_validation .content .summary`).html(
  ...
  function updateIngestStatusBlock(fileId, status) {
    ...
    if (stageStatusArr.length > 0) stageStatus = stageStatusArr[0];
    if (stageStatus) {
      $(`#fileIngestCard table tbody tr#${fileId} td.label_status .progress-loader`).hide();
      $(`#fileIngestCard table tbody tr#${fileId} td.label_status .content`).html(`
      <span class="badge bg-soft-${stageStatus.indicatorVariant} text-${stageStatus.indicatorVariant} p-1 shadow-none">${stageStatus.text}</span>`).show();
    }
  }
    ...
    if (stageStatus) {
      switch (status) {
        case IMPORT_STATUS_ONGOING: {
          $(`#fileIngestCard table tbody tr#${fileId} td.label_progress .progress-loader`).hide();
          $(`#fileIngestCard table tbody tr#${fileId} td.label_progress .content`).show();
          break;
        }
        case IMPORT_STATUS_FAILED: {
          $(`#fileIngestCard table tbody tr#${fileId} td.label_progress .progress-loader`).hide();
          $(`#fileIngestCard table tbody tr#${fileId} td.label_progress .content`).html(`
             <span class="badge bg-soft-${stageStatus.indicatorVariant} text-${stageStatus.indicatorVariant} p-1 shadow-none">${stageStatus.text}</span>`).show();
          break;
        }
        case IMPORT_STATUS_COMPLETED: {
          $(`#fileIngestCard table tbody tr#${fileId} td.label_progress .progress-loader`).hide();
          $(`#fileIngestCard table tbody tr#${fileId} td.label_progress .content`).show();
          $(`#fileIngestCard table tbody tr#${fileId} td.label_progress .content .progress-bar`).removeClass('progress-bar-animated');
          break;
        }
        default:
          break;
  ...
  function updateIngestActionBlock(fileId, status) {
    ...
    if (stageStatus) {
      switch (status) {
        case IMPORT_STATUS_ONGOING: {
          $(`#fileIngestCard table tbody tr#${fileId} td.label_action .progress-loader`).hide();
          $(`#fileIngestCard table tbody tr#${fileId} td.label_action .content`).show();
          break;
        }
        case IMPORT_STATUS_FAILED: {
          $(`#fileIngestCard table tbody tr#${fileId} td.label_action .progress-loader`).hide();
          $(`#fileIngestCard table tbody tr#${fileId} td.label_action .content`).html(`
              <div class="btn-group btn-group-sm" style="float: none;">
                <button type="button" class="failedFileDiscard tabledit-edit-button btn btn-danger" style="float: none;"><span class="">Discard</span></button>
              </div>`).show();
          break;
  ...
  function updateIngestionProcess(fileId) {
    ...
    if (fileEntry.status === IMPORT_STATUS_COMPLETED) {
      ...
      $(`#fileIngestCard table tbody tr#${fileId} td.label_validation .progress-loader`).hide();

      $(`#fileIngestCard table tbody tr#${fileId} td.label_validation .content`).show();
      $(`#fileIngestCard table tbody tr#${fileId} td.label_validation .content .title`).text('Passed');
      $(`#fileIngestCard table tbody tr#${fileId} td.label_validation .content .summary`).html(
        '<small class="summary_passed">No Issues Detected</small>');
      ...
      $(`#fileIngestCard table tbody tr#${fileId} td.label_validation .content`).show();
      $(`#fileIngestCard table tbody tr#${fileId} td.label_validation .content .title`).text('Failed');
      $(`#fileIngestCard table tbody tr#${fileId} td.label_validation .content .summary`).html(
        `<a href="javascript: void(0);" class="issueValidationPreview" file-id="${fileId}">
          <small class="text-danger">${fileEntry.errors.length} Issues Detected</small>
        </a>`);
  ...
  function ingestFile(ingestMode, fileId) {
    if (ingestMode === IMPORT_INGEST_MODE_MANUAL) {
      ...
      for (let i in fileImportQueue) {
        ...
        if (fileImportQueue[i].stage === IMPORT_STAGE_INGEST &&
          (fileImportQueue[i].status === IMPORT_STATUS_PENDING || fileImportQueue[i].status === IMPORT_STATUS_ONGOING)) {
  ...
  function registerLedgerFiles() {
    ...
    if ((codeISO3 && tradeType) && (tradeYear && ingestMode)) {
      ...
      let taxonomyIdArr = taxonomyStandards.filter(taxonomy =>
        (taxonomy.code_iso_3 == codeISO3.toUpperCase().trim() && taxonomy.trade == tradeType.toUpperCase().trim()));
  ...
  function examineTradeFileCompatibilityChecks(fileEntry, results, issues) {
    ...
    headerFields.forEach(headerField => {
      let dataTypeArr = taxonomyStandard.fields.dataTypes.filter(type => type.field === headerField);
    ...
    for (let i in fileImportQueue) {
      if (fileImportQueue[i]._id === fileEntry._id) {
        if (fileImportQueue[i].stage === IMPORT_STAGE_EXAMINE && fileImportQueue[i].status === IMPORT_STATUS_PENDING) {
  ...
  function processFileDataStageIngest(fileId, stage, status, errors, pollerId) {
    if (stage === IMPORT_STAGE_INGEST) {
      ...
      if (processDataStageFile &&
        (processDataStageFile.status === IMPORT_STATUS_COMPLETED || processDataStageFile.status === IMPORT_STATUS_FAILED)) {
  ...
  function sendFileForIngestion(ingestMode, fileId) {
    ...
    for (let i in fileImportQueue) {
      if (fileImportQueue[i].stage === IMPORT_STAGE_UPLOAD && fileImportQueue[i].status === IMPORT_STATUS_COMPLETED) {
  ...
  function sendFileForUpload() {
    ...
    for (let i in fileImportQueue) {
      if (fileImportQueue[i].stage === IMPORT_STAGE_EXAMINE && fileImportQueue[i].status === IMPORT_STATUS_COMPLETED) {
  ...
  function sendFileForValidation() {
    ...
    for (let i in fileImportQueue) {
      if (fileImportQueue[i].stage === IMPORT_STAGE_REGISTER && fileImportQueue[i].status === IMPORT_STATUS_COMPLETED) {
  ...
  function sendFileForRegistration() {
    let fileImportQueuePendingRegistration = fileImportQueue.filter(file =>
      (file.stage === IMPORT_STAGE_REGISTER && file.status === IMPORT_STATUS_PENDING));
  ...
  function discardImportFile(fileId) {
    let fileEntry = getFileImportQueueEntry(fileId);
    fileImportQueue = fileImportQueue.filter(queueFile => queueFile._id != fileId);
  ...
  function prepareImportProcess() {
    $('html, body').animate({
      scrollTop: ($("#importProcessSection").offset().top - $("#topnav").height())
    }, 1000);
    fileImportQueue.forEach(uploadFile => {
      if (uploadFile.stage === IMPORT_STAGE_REGISTER && uploadFile.status === IMPORT_STATUS_COMPLETED) {
  ...
  function fetchFileDataStageAPIHandler(fileId, dataStage, pollerId) {
    $.ajax({
      url: API_HOST.concat(ENDPOINT_FETCH_FILE_DATA_STAGE, QUERY_PARAMS_INDICATOR,
        'file_id', QUERY_PARAMS_VALUE_ASSIGNER, fileId, QUERY_PARAMS_SEPARATOR,
        'data_stage', QUERY_PARAMS_VALUE_ASSIGNER, dataStage),
      type: 'GET',
      success: function (payload) {
        switch (dataStage) {
          ...
          case IMPORT_STAGE_INGEST:
            processFileDataStageIngest(payload.data.file_id, dataStage, payload.data.status_code, payload.data.errors, pollerId);
            break;
          default:
            break;
        }
  ...
  function ingestDataFileAPIHandler(file) {
    ...
    $.ajax({
      url: API_HOST.concat(ENDPOINT_INGEST_FILE_DATA).replace(':fileId', file._id),
      type: 'PUT',
      data: {
  ...
  function updateFileDataStageAPIHandler(file) {
    ...
    $.ajax({
      url: API_HOST.concat(ENDPOINT_UPDATE_FILE_DATA_STAGE).replace(':fileId', file._id),
      type: 'PUT',
      data: {
        payload: JSON.stringify({
          stage: dataStage
  ...
  function verifyFilesExistenceAPIHandler(filesParams) {
    ...
    Swal.showLoading();
    $.ajax({
      url: API_HOST.concat(ENDPOINT_VERIFY_FILES_EXISTENCE, QUERY_PARAMS_INDICATOR,
        'files', QUERY_PARAMS_VALUE_ASSIGNER, filesParams),
      type: 'GET',
      success: function (payload) {
        graceCloseSwal();
