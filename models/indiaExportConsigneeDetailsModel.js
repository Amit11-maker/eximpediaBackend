...
async function addOrUpdateCustomerRequest(requestData) {
  logger.log("Method = addOrUpdateCustomerRequest, Entry");
  try {
    let requestPayload = {};
    ...
    logger.log(
      `Method = addOrUpdateCustomerRequest, Error =  ${JSON.stringify(error)}`
    );
    throw error;
  } finally {
    logger.log("Method = addOrUpdateCustomerRequest, Exit");
  }
}
...
async function deleteCustomerRequest(requestData) {
  ...
  try {
    ...
    updatedRequestData.requested_shipments =
      updatedRequestData.requested_shipments.filter((requestedShipment) => {
        return requestedShipment.bill_number != shipmentBillNumber;
      });

    updatedRequestData.recordData = updatedRequestData.recordData.filter(
      (requestedShipment) => {
...
async function getRequestsList(offset, limit) {
  ...
  try {
    ...
    const recordsFiltered = await MongoDbHandler.getDbInstance()
      .collection(MongoDbHandler.collections.shipment_request_details)
      .aggregate(countRecordsAggregationExpression)
      .toArray();

    return { data: requestListData, recordsFiltered: recordsFiltered[0].count };
  } catch (error) {
    logger.log(`Method = getRequestsList, Error = ",${JSON.stringify(error)}`);
    throw error;
  } finally {
...
async function getUserRequestData(userId) {
  logger.log("Method = getUserRequestData, Exit");
  try {
    ...
    logger.log(
      `"Method = getUserRequestData, Error = ", ${JSON.stringify(error)}`
    );
    throw error;
  } finally {
    logger.log("Method = getUserRequestData, Exit");
...
async function updateRequestResponse(userRequestData, shipmentNumber) {
  try {
    userRequestData.requested_shipments =
      userRequestData.requested_shipments.filter((requestedShipment) => {
    ...
    logger.log(`"Method = , Error = ${JSON.stringify(error)}`);
    res.status(500).json({
      data: error,
    });
  } finally {
...
async function getShipmentData(shipmentNumber) {
  logger.log("Method = getShipmentData, Exit");
  try {
    const shipmentData = await MongoDbHandler.getDbInstance()
      .collection(MongoDbHandler.collections.consignee_shipment_details)
      .find({ shipment_number: shipmentNumber })
      .toArray();

    return shipmentData[0];
  } catch (error) {
    logger.log(`"Method = getShipmentData, Error = ${JSON.stringify(error)}`);
    throw error;
  } finally {
    logger.log("Method = getShipmentData, Exit");
  }
...
async function updateShipmentRequestLimits(
  accountId,
  updatedShipmentRequestLimits
) {
  const matchClause = {
    account_id: ObjectID(accountId),
    max_request_shipment_count: {
